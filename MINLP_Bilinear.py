...
Ojo=0
npl = 100 #número de pontos da linearização
qgl = np.linspace(0,350000,npl)
# Allocando as matrices
qo_well1 = np.zeros([8,npl])
qo_well2 = np.zeros([8,npl])
qo_well3 = np.zeros([8,npl])
qo_well4 = np.zeros([8,npl])
qo_well5 = np.zeros([8,npl])
qo_well6 = np.zeros([8,npl])
qo_well7 = np.zeros([8,npl])
qo_well8 = np.zeros([8,npl])
...
# Valores minimos de cada segmento de gl
glp_min = qgl[0:npl-1] 

# Valores maximos de cada segmento de gl
glp_max = qgl[1:npl] 

# Valores de qo minimos de cada segmento de gl
qop_min_well1 = qo_well1[:,0:npl-1]
qop_min_well2 = qo_well2[:,0:npl-1]
qop_min_well3 = qo_well3[:,0:npl-1]
qop_min_well4 = qo_well4[:,0:npl-1]
qop_min_well5 = qo_well5[:,0:npl-1]
qop_min_well6 = qo_well6[:,0:npl-1]
qop_min_well7 = qo_well7[:,0:npl-1]
qop_min_well8 = qo_well8[:,0:npl-1]

# Valores de qo maximos de cada segmento de gl
qop_max_well1 = qo_well1[:,1:npl]
qop_max_well2 = qo_well2[:,1:npl]
qop_max_well3 = qo_well3[:,1:npl]
qop_max_well4 = qo_well4[:,1:npl]
qop_max_well5 = qo_well5[:,1:npl]
qop_max_well6 = qo_well6[:,1:npl]
qop_max_well7 = qo_well7[:,1:npl]
qop_max_well8 = qo_well8[:,1:npl]
...
'''Variaveis do modelo'''
# Variables continuas para gl
x_gl_well1 = m.Var(value=0.5,lb=0,ub=1)
x_gl_well2 = m.Var(value=0.5,lb=0,ub=1)
x_gl_well3 = m.Var(value=0.5,lb=0,ub=1)
x_gl_well4 = m.Var(value=0.5,lb=0,ub=1)
x_gl_well5 = m.Var(value=0.5,lb=0,ub=1)
x_gl_well6 = m.Var(value=0.5,lb=0,ub=1)
x_gl_well7 = m.Var(value=0.5,lb=0,ub=1)
x_gl_well8 = m.Var(value=0.5,lb=0,ub=1)

# Variaveis enteiras para gl
...
# Variaveis enteiras para whp
y_whp_well1 = m.Array(m.Var,8,value=0,lb=0,ub=1,integer=True)
y_whp_well2 = m.Array(m.Var,8,value=0,lb=0,ub=1,integer=True)
y_whp_well3 = m.Array(m.Var,8,value=0,lb=0,ub=1,integer=True)
y_whp_well4 = m.Array(m.Var,8,value=0,lb=0,ub=1,integer=True)
y_whp_well5 = m.Array(m.Var,8,value=0,lb=0,ub=1,integer=True)
y_whp_well6 = m.Array(m.Var,8,value=0,lb=0,ub=1,integer=True)
y_whp_well7 = m.Array(m.Var,8,value=0,lb=0,ub=1,integer=True)
y_whp_well8 = m.Array(m.Var,8,value=0,lb=0,ub=1,integer=True)

# Vazao de gas lift
qgl_well1 =  m.Var(value=150000,lb=0,ub=350000)
qgl_well2 =  m.Var(value=150000,lb=0,ub=350000)
qgl_well3 =  m.Var(value=150000,lb=0,ub=350000)
qgl_well4 =  m.Var(value=150000,lb=0,ub=350000)
qgl_well5 =  m.Var(value=150000,lb=0,ub=350000)
qgl_well6 =  m.Var(value=150000,lb=0,ub=350000)
qgl_well7 =  m.Var(value=150000,lb=0,ub=350000)
qgl_well8 =  m.Var(value=150000,lb=0,ub=350000)
...
# Vector de vazao de oleo em cada nivel de pressao interpolada para gl real
qo_gl_real_well1 = m.Array(m.Var,8,lb=0)
qo_gl_real_well2 = m.Array(m.Var,8,lb=0)
qo_gl_real_well3 = m.Array(m.Var,8,lb=0)
qo_gl_real_well4 = m.Array(m.Var,8,lb=0)
qo_gl_real_well5 = m.Array(m.Var,8,lb=0)
qo_gl_real_well6 = m.Array(m.Var,8,lb=0)
qo_gl_real_well7 = m.Array(m.Var,8,lb=0)
qo_gl_real_well8 = m.Array(m.Var,8,lb=0)
...
# Vazao de agua produzida total
qw_total = m.Var(lb=0)

# Vazao total de liquido 
ql_total = m.Var(lb=0)	
			
# Vazao de gaz produzido total
qg_total = m.Var(lb=0)
	
# Vazao de gas lift total
...
'''Equaçoes do modelo'''
# Suma das variaveis inteiras para gas lift
...
# Suma das variaveis inteiras para whp
...
# Vazão de gas lift
m.Equation(qgl_well1==m.sum(y_gl_well1*(glp_min*(1-x_gl_well1)+glp_max*x_gl_well1)))
m.Equation(qgl_well2==m.sum(y_gl_well2*(glp_min*(1-x_gl_well2)+glp_max*x_gl_well2)))
m.Equation(qgl_well3==m.sum(y_gl_well3*(glp_min*(1-x_gl_well3)+glp_max*x_gl_well3)))
m.Equation(qgl_well4==m.sum(y_gl_well4*(glp_min*(1-x_gl_well4)+glp_max*x_gl_well4)))
m.Equation(qgl_well5==m.sum(y_gl_well5*(glp_min*(1-x_gl_well5)+glp_max*x_gl_well5)))
m.Equation(qgl_well6==m.sum(y_gl_well6*(glp_min*(1-x_gl_well6)+glp_max*x_gl_well6)))
m.Equation(qgl_well7==m.sum(y_gl_well7*(glp_min*(1-x_gl_well7)+glp_max*x_gl_well7)))
m.Equation(qgl_well8==m.sum(y_gl_well8*(glp_min*(1-x_gl_well8)+glp_max*x_gl_well8)))
...
# Vazao de oleo real para cada pressao
for i in range(8):
    m.Equation(qo_gl_real_well1[i] == m.sum(y_gl_well1*(qop_min_well1[i]*(1-x_gl_well1)+qop_max_well1[i]*x_gl_well1)))
    m.Equation(qo_gl_real_well2[i] == m.sum(y_gl_well2*(qop_min_well2[i]*(1-x_gl_well2)+qop_max_well2[i]*x_gl_well2)))
    m.Equation(qo_gl_real_well3[i] == m.sum(y_gl_well3*(qop_min_well3[i]*(1-x_gl_well3)+qop_max_well3[i]*x_gl_well3)))
    m.Equation(qo_gl_real_well4[i] == m.sum(y_gl_well4*(qop_min_well4[i]*(1-x_gl_well4)+qop_max_well4[i]*x_gl_well4)))
    m.Equation(qo_gl_real_well5[i] == m.sum(y_gl_well5*(qop_min_well5[i]*(1-x_gl_well5)+qop_max_well5[i]*x_gl_well5)))
    m.Equation(qo_gl_real_well6[i] == m.sum(y_gl_well6*(qop_min_well6[i]*(1-x_gl_well6)+qop_max_well6[i]*x_gl_well6)))
    m.Equation(qo_gl_real_well7[i] == m.sum(y_gl_well7*(qop_min_well7[i]*(1-x_gl_well7)+qop_max_well7[i]*x_gl_well7)))
    m.Equation(qo_gl_real_well8[i] == m.sum(y_gl_well8*(qop_min_well8[i]*(1-x_gl_well8)+qop_max_well8[i]*x_gl_well8)))

# Vazao de oleo
m.Equation(qo[0] == m.sum(y_whp_well1*qo_gl_real_well1))
m.Equation(qo[1] == m.sum(y_whp_well2*qo_gl_real_well2))
m.Equation(qo[2] == m.sum(y_whp_well3*qo_gl_real_well3))
m.Equation(qo[3] == m.sum(y_whp_well4*qo_gl_real_well4))
m.Equation(qo[4] == m.sum(y_whp_well5*qo_gl_real_well5))
m.Equation(qo[5] == m.sum(y_whp_well6*qo_gl_real_well6))
m.Equation(qo[6] == m.sum(y_whp_well7*qo_gl_real_well7))
m.Equation(qo[7] == m.sum(y_whp_well8*qo_gl_real_well8))
...
m.Equation(qo_total>=6620.1174725-5) 
Ojo=0
